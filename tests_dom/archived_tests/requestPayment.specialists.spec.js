/**
 * Request a payment for Specialists
 */
...
const addAnotherCostDropdown = Selector(
  'div.request-payment__costs-for-specialists-select-wrapper.is-cost select'
)
const otherCostWrapper = Selector('div.is-other-cost')

const addAnotherProcedure = Selector('a.is-add-another-procedure')

const totalCostLabel = Selector('span.request-payment__costs-for-specialists-total-label')
...
fixture `***** Provider portal Request a payment Specialists tests`
  .page(config.domTestRootUrl)

test('Request payment for Specialists -- Invoice details', async t => {
  H.BeginTest('Request payment for Specialists -- Invoice details')
  ...
  await t
    ...
    .expect(costsSectionForSpecialists.exists).ok('Should display Specialists Costs section')
...
test('Request payment for Specialists -- Costs', async t => {
  H.BeginTest('Request payment for Specialists -- Costs')

  await defaultLoginSteps(t)

  H.StepDescription('Sections should exist')
  await t
    .expect(costsSectionForSpecialists.exists).ok('Should display Costs details section')
