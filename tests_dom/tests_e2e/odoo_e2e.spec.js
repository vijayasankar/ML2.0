...
test('Create purchase order ', async t => {
  ...
  const untaxedAmount = Selector('.o_form_field_number').nth(2)
  const taxes = Selector('.o_form_field_number').nth(3)
  const totalAmount = Selector('.o_form_field_number').nth(4)
  await t
...
//      .debug()
      ...
      //.expect(subTotalPrice.value).eql('396.1900')
       //.expect(untaxedAmount.value).eql('396.1900')
      // .expect(taxes.value).eql('$59.43')
      // .expect(totalAmount.value).eql('$455.62')
      .click(saveQuotation)
      .wait(3000)
